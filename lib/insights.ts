 ...
 * The future aggregation layer must validate IDs against non-demo experiences.
 */
export function usableInsights(insights: PlaceInsights | null | undefined): PlaceInsights | null {
  ...
  const result = { ...insights, vibe: supported(insights.vibe), bestFor: supported(insights.bestFor), positives: supported(insights.positives), complaints: supported(insights.complaints), popularItems: supported(insights.popularItems), preferences: supported(insights.preferences) };
  return [result.vibe, result.bestFor, result.positives, result.complaints, result.popularItems, result.preferences].some(items => items.length) ? result : null;
}